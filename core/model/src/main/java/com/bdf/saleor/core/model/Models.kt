...
data class AddressDraft(
    ...
    fun toAddress(id: String, isDefaultShipping: Boolean = false) = Address(
        id = id,
        firstName = firstName,
...
fun Address.toDraft() = AddressDraft(
    firstName = firstName,
    lastName = lastName,
    companyName = companyName,
    streetAddress1 = streetAddress1,
...
class SaleorException(message: String) : Exception(message)
...
fun CheckoutSession.payableTotal(): Double? {
    ...
    val reconstructed = kotlin.math.max(0.0, fromLines + shippingAmount - discountAmount)
...
fun calculatePayAmount(
    ...
    if (checkout.authorizeStatus == CheckoutAuthorizeStatus.PARTIAL) {
        ...
        if (balance != null && balance > 0) return kotlin.math.min(balance, afterPoints)
    }
    return afterPoints
}
