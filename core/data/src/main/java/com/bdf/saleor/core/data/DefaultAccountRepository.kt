...
class DefaultAccountRepository @Inject constructor(
    ...
    override suspend fun createAddress(draft: AddressDraft, defaultKind: AddressKind?): Result<String?> {
        ...
        if (errors.isNotEmpty()) {
            return Result.failure(SaleorException(errors.firstOrNull()?.message ?: "주소 추가에 실패했습니다"))
        }
    ...
    override suspend fun updateAddress(id: String, draft: AddressDraft): Result<String?> {
        ...
        if (errors.isNotEmpty()) {
            return Result.failure(SaleorException(errors.firstOrNull()?.message ?: "주소 수정에 실패했습니다"))
        }
        return Result.success(null)
    }
    ...
    override suspend fun deleteAddress(id: String): Result<String?> {
        ...
        if (errors.isNotEmpty()) {
            return Result.failure(SaleorException(errors.firstOrNull()?.message ?: "주소 삭제에 실패했습니다"))
        }
        return Result.success(null)
    }
    ...
    override suspend fun setDefaultAddress(id: String, kind: AddressKind): Result<String?> {
        ...
        if (errors.isNotEmpty()) {
            return Result.failure(SaleorException(errors.firstOrNull()?.message ?: "기본 주소 설정에 실패했습니다"))
        }
        return Result.success(null)
    }
    ...
    override suspend fun requestAccountDeletion(): Result<String?> {
        ...
        if (errors.isNotEmpty()) {
            return Result.failure(SaleorException(errors.firstOrNull()?.message ?: "계정 삭제 요청에 실패했습니다"))
        }
        return Result.success("삭제 확인 메일을 보냈습니다. 메일함에서 계정을 삭제하세요.")
    }
}
